...
- (void)_initViews
{
    for (int i = 0; i < self.images.count; i++) {
        ...
        [self createImageViews:i withImageName:image];
    }
...
- (void)createImageViews:(int)i withImageName:(UIImage *)image
{
    ...
    imgView.frame = CGRectMake(singleWidth*i, 0, self.scrollView.frame.size.height, self.scrollView.frame.size.height);
...
- (void)deleteAction:(UIButton *)button
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"photoDelete" object:[NSString stringWithFormat:@"%d",index]];
